...
# *.po POFILE
#
# Change from the Stackoverflow version:
# - msguniq --force-po -o "${TEMP}/other" --unique "${OTHER}" &
# + msguniq --no-location --force-po -o "${TEMP}/other" --unique "${OTHER}" &
# to avoid having repeated and incorrect source location marks in the result
##########################################################################
# CONFIG:
