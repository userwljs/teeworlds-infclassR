...
void CTurret::AttackTargets()
{
	...
	for(TEntityPtr<CInfClassCharacter> pChr = GameWorld()->FindFirst<CInfClassCharacter>(); pChr; ++pChr)
	{
		...
		if(Len < (float)Config()->m_InfTurretRadarRange) // 800
		{
			...
			{
			case LASER:
				CInfClassLaser::MakeLaser(GameServer(), m_Pos, Direction, GameServer()->Tuning()->m_LaserReach, m_Owner, Config()->m_InfTurretDmgHealthLaser, EInfclassWeapon::TURRET_INSTALL_KIT);
				m_ammunition--;
				break;
			case PLASMA:
			{
