...
void CCharacterCore::Init(CWorldCore *pWorld, const CCollision *pCollision, CTeamsCore *pTeams)
{
	m_pWorld = pWorld;
...
void CCharacterCore::Tick(bool UseInput, const CParams *pParams)
{
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		[[maybe_unused]] bool GoingThroughTele = false;
		int Hit = m_pCollision->IntersectLineHook(m_HookPos, NewPos, &NewPos, nullptr);
		if(Hit)
		{
			if(Hit == TILE_NOHOOK)
				GoingToRetract = true;
